...
@section('content')
    <div class="container-fluid pt-4 px-4">
        <div class="row g-4">
            ...
            <form action="{{route('product.store')}}" method="POST" enctype="multipart/form-data" class="d-flex gap-4">
                @csrf
                        ...
                        <div class="mb-3">
                            ...
                            <select class="form-control" type="file" name="category_id" id="formFile">
                                @foreach($categories as $category)
                                    <option
                                        @selected(old('category_id')==$category->id) value="{{$category->id}}">{{$category->title_az}}</option>
